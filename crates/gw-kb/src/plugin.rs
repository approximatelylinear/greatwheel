            ...
            move |args, kwargs| {
                ...
                async move {
                    ...
                    Ok(serde_json::to_value(hits).map_err(|e| {
                        PluginError::HostFunction(format!("kb.search serialize: {e}"))
                    })?)
                }
            },
        );
        ...
        ctx.register_host_fn_async(
            ...
            move |args, kwargs| {
                ...
                async move {
                    ...
                    let seeds_k =
                        get_optional_usize(&args, &kwargs, 2, "seeds").unwrap_or(3);
                    let hops =
                        get_optional_usize(&args, &kwargs, 3, "hops").unwrap_or(3);
                    let decay = get_optional_f32(&args, &kwargs, 4, "decay").unwrap_or(0.5);

                    // 1. Embed query
                    let query_vec = stores.embedder.embed_one(&query).map_err(|e| {
                        PluginError::HostFunction(format!("kb.explore embed: {e}"))
                    })?;
                    // 2. Seed topics
                    let seed_topics = nearest_topics_to_query(&stores.pg, &query_vec, seeds_k)
                        .await
                        .map_err(|e| {
                            PluginError::HostFunction(format!("kb.explore seeds: {e}"))
                        })?;
                    ...
                    .await
                    .map_err(|e| {
                        PluginError::HostFunction(format!("kb.explore spread: {e}"))
                    })?;
            ...
            move |args, kwargs| {
                ...
                async move {
                    ...
                    let summary_pair = fetch_summary(&stores.pg, topic.topic_id)
                        .await
                        .map_err(|e| {
                            PluginError::HostFunction(format!("kb.topic summary: {e}"))
                        })?;
                    let neigh = neighbors_of(&stores.pg, topic.topic_id, 25)
                        .await
                        .map_err(|e| {
                            PluginError::HostFunction(format!("kb.topic neighbors: {e}"))
                        })?;
        ...
        ctx.register_host_fn_async(
            ...
            move |args, kwargs| {
                ...
                async move {
                    ...
                    let rows = list_topic_summaries(&stores.pg, limit).await.map_err(|e| {
                        PluginError::HostFunction(format!("kb.topics: {e}"))
                    })?;
